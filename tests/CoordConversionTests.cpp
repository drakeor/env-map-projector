...
#include "../src/Coordinates/Point3d.h"
...
TEST_CASE( "CoordConversions", "[coords-conversions]" ) {
    ...
    SECTION( "case [1,0,0]" ) {
        ...
        // at this point
        Point3d<float> rebuiltPoint =  CoordConversions<float>::SphericalToCartesian(spherePoint);
    ...
    SECTION( "case [-1,-0.25,0.85]" ) {
        ...
        Point3d<float> cartPoint({-1.0f, -0.25f, 0.85f});
        PointSphere<float> spherePoint = CoordConversions<float>::CartesianToSpherical(cartPoint);
